...
void saveToDisk(std::unique_ptr<MetadataBuilder> builder) {
  ...
  for (int slot = 0; slot < nSlots; slot++) {
    std::cout << "Saving the updated partition table "
              << UpdatePartitionTable(opener, "super", *newMetadata, slot) << " for slot " << slot
              << std::endl;
  }
}
...
int main(int argc, char** argv) {
  ...
  if (strcmp(argv[1], "create") == 0) {
    ...
    std::cout << "Creating dm partition for " << partName << " answered " << dmCreateRes << " at "
              << dmPath << std::endl;
    exit(0);
    ...
    if (argc != 4) {
      std::cerr << "Usage: " << argv[0] << " replace <original partition name> <new partition name>"
                << std::endl;
      std::cerr << "This will delete <new partition name> and rename <original partition name> to "
                   "<new partition name>"
                << std::endl;
      exit(1);
    }
    ...
    if (srcPartition == nullptr) {
      srcPartition =
          builder->FindPartition(src + ::android::base::GetProperty("ro.boot.slot_suffix", ""));
    }
    auto dstPartition = builder->FindPartition(dst);
    if (dstPartition == nullptr) {
      dstPartition =
          builder->FindPartition(dst + ::android::base::GetProperty("ro.boot.slot_suffix", ""));
    }
    ...
    for (unsigned i = 0; i < extents.size(); i++) {
      ...
      if (linear != nullptr) {
        auto copyLinear = std::make_unique<LinearExtent>(
            linear->num_sectors(), linear->device_index(), linear->physical_sector());
    ...
    if (groupAllocatable > superFreeSpace || maxSize == 0) groupAllocatable = superFreeSpace;
    ...
    if (svc1_1 == nullptr) {
      std::cerr << "Couldn't get a bootcontrol HAL. You can clear cow only on V AB devices"
                << std::endl;
      return 1;
    }
    auto mergeStatus = svc1_1->getSnapshotMergeStatus();
    if (mergeStatus != ::android::hardware::boot::V1_1::MergeStatus::NONE) {
      std::cerr << "Merge status is NOT none, meaning a merge is pending. Clearing COW isn't safe"
                << std::endl;
      return 1;
    }
#endif
